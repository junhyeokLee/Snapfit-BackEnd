...
public interface AlbumRepository extends JpaRepository<AlbumEntity, Long> {
  ...
  /**
   * 특정 사용자가 저장한 앨범 목록 (순서 + 최신순)
   */
  List<AlbumEntity> findByUserIdOrderByOrdersAscCreatedAtDesc(String userId);
  ...
  @Query("""
      select distinct a
      from AlbumEntity a
      left join AlbumMemberEntity m on m.album = a
      where (a.userId = :userId)
         or (m.userId = :userId and m.status = com.snapfit.snapfitbackend.domain.album.entity.AlbumMemberStatus.ACCEPTED)
      order by a.orders asc, a.createdAt desc
      """)
