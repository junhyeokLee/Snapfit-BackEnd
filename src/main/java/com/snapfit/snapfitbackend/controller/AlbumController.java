...
public class AlbumController {
        ...
        public ResponseEntity<List<AlbumListResponse>> getMyAlbums(
                ...
                List<AlbumListResponse> response = albums.stream()
                                .map(a -> AlbumListResponse.builder()
                                                ...
                                                .coverLayersJson(a.getCoverLayersJson()) // 커버 레이어 JSON
                                                .coverTheme(a.getCoverTheme())
        ...
        /**
         * 앨범 수정 (소유자만)
         * - Body: CreateAlbumRequest와 동일 (ratio, coverLayersJson, coverImageUrl,
         * coverThumbnailUrl, userId)
         */
        ...
        public ResponseEntity<AlbumDetailResponse> getAlbumDetail(
                ...
                List<AlbumDetailResponse.AlbumPageSummaryResponse> pageResponses = pages.stream()
                                .map(page -> AlbumDetailResponse.AlbumPageSummaryResponse.builder()
                                                .pageId(page.getId())
                                                .pageNumber(page.getPageNumber())
                                                .imageUrl(page.getPreviewUrl() != null ? page.getPreviewUrl()
                                                                : page.getImageUrl())
                                                .thumbnailUrl(page.getThumbnailUrl())
                                                .build())
                                .collect(Collectors.toList());
         ...
         * 앨범 편집 잠금 (진입 시)
         */
        @Operation(summary = "앨범 편집 잠금")
        @PostMapping("/{albumId}/lock")
        public ResponseEntity<Void> lockAlbum(
                        @PathVariable Long albumId,
                        @RequestParam String userId) {
                try {
                        albumService.lockAlbum(albumId, userId);
                        return ResponseEntity.ok().build();
         ...
         * 앨범 편집 잠금 해제 (이탈 시)
         */
        @Operation(summary = "앨범 편집 잠금 해제")
        @PostMapping("/{albumId}/unlock")
        public ResponseEntity<Void> unlockAlbum(
                        @PathVariable Long albumId,
                        @RequestParam String userId) {
                albumService.unlockAlbum(albumId, userId);
                return ResponseEntity.ok().build();
        }

        // DTO 내부 정의 (이 파일 하단이나 별도 파일로 분리 가능)
        @lombok.Data
        public static class ReorderAlbumsRequest {
